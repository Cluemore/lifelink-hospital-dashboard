import { Ambulance, ArrowUpRight, Clock3, MapPin, Stethoscope } from 'lucide-react';
import { Link } from 'react-router-dom';
import type { Emergency } from '../../types';
import { readableStatus, timeAgo } from '../../utils/format';
import { Badge } from '../ui/Badge';

const progressByStatus: Record<string, number> = { ACCEPTED: 15, AMBULANCE_ASSIGNED: 32, EN_ROUTE: 52, PATIENT_PICKED_UP: 72, ARRIVED: 90 };
...
export function OngoingCaseCard({ emergency }: { emergency: Emergency }) {
  ...
  return <article className="ongoing-card">
    <div className="ongoing-card__top"><div><span className="emergency-card__id">{emergency.id}</span><Badge tone={tone}>{emergency.priority.level}</Badge></div><span className="ongoing-status"><i />{readableStatus(emergency.status)}</span></div>
