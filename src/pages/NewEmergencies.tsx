...
export function NewEmergencies() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    emergencyApi.getNew(currentHospital.id).then(setItems).catch((caught: unknown) => setError(caught instanceof Error ? caught.message : 'Please check the LifeLink backend connection.')).finally(() => setLoading(false));
